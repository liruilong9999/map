...
#define PLUGIN_CONF_PATH_BASE QString(qApp->applicationDirPath() + "/config/")
...
void PluginManager::setConfigPath(QString path, bool isFullPath)
{
    if (isFullPath)
    {
        m_configFile = path;
    ...
    {
        m_configFile = PLUGIN_CONF_PATH_BASE + path + ".xml";
    }
}
...
bool PluginManager::loadPlugin(QString & filePath)
{
    ...
    // 检测依赖
    if (!m_pluginData->check(filePath))
    {
        strcons = QString("插件(%1)加载失败，依赖检查不过关！").arg(filePath);
        LOG_WARN(strcons);
        return false;
            ...
            if (fileInfo.baseName() == m_pluginData->m_loadOrder[i])
            {
                ...
                if (loader)
                {
                    ...
                    if (plugin)
                    {
                        ...
                        {
                            QString strcons = QString("清理插件(%1)失败").arg(fileInfo.baseName());
                            LOG_WARN(strcons);
                            // qDebug() << strcons;
                        }
                    }
